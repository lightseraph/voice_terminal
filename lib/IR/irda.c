...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if (htim == &htim2)
    {
        ...
        {
            dval = HAL_TIM_ReadCapturedValue(&htim2, TIM_CHANNEL_1);                               /* 读取CCR1也可以清CC1IF标志位 */
            __HAL_TIM_SET_CAPTUREPOLARITY(&htim2, TIM_CHANNEL_1, TIM_INPUTCHANNELPOLARITY_RISING); /* 设置为上升沿捕获 */
